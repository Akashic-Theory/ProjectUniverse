...
@onready var graphics_button_list = {
	"fullscreen": find_child("AssignFullscreen") as BaseButton
}
...
func _ready():
	...
	graphics_button_list["fullscreen"].button_pressed = Game.settings.graphics["fullscreen"]
...
func _toggle_graphics_setting(val: bool, tag: String):
	...
	if event is InputEventKey:
		...
		if event_list.size() <= cur_bind_index:
			...
			if event_is_duplicate && event_list[cur_bind_index].keycode != event.keycode:
				...
				controls_button_list[cur_action + str(cur_bind_index)].set_text("")
			else:
				Game.settings.rebind_control(cur_action, event_list[cur_bind_index], event)
		
		# Update the button text
		if !event_is_duplicate:
			controls_button_list[cur_action + str(cur_bind_index)].set_text(OS.get_keycode_string(event.keycode))
		is_assigning = false
